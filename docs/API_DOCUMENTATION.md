...
## 3. CDN & Edge DNS API Reference

**Base Path:** `/cdn/4.0`

### 3.1 List CDN Domains
```http
GET /cdn/4.0/domains
```
...
### 3.2 Register CDN Domain & DNS Zone
```http
POST /cdn/4.0/domains/dns-service
```
...
#### Update DNS Record
```http
PUT /cdn/4.0/domains/{domain}/dns-records/{record_id}
```

#### Delete DNS Record
```http
DELETE /cdn/4.0/domains/{domain}/dns-records/{record_id}
```

---

### 3.4 Caching & SSL Settings

#### Purge Edge Cache
```http
POST /cdn/4.0/domains/{domain}/caching/purge
```
**Request Body:**
```json
{
...
```http
PUT /cdn/4.0/domains/{domain}/ssl
```
**Request Body:**
```json
{
...
## 4. Object Storage (S3-Compatible) API Reference

**Base Path:** `/storage/v1`  
**S3 Protocol Compatible Base URL:** `https://s3.ir-thr-at1.arvanstorage.ir`
...
### 4.3 Manage S3 Access Keys (User Credentials)

#### Generate New S3 Access Key Pair
```http
POST /storage/v1/user/keys
```
...
{
  "data": {
    "access_key": "ARVAN_AKIA_XXXXXXXXXXXXXXXX",
    "secret_key": "ARVAN_SECRET_YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY",
    "description": "WordPress Media Storage S3 Key",
    "created_at": "2026-08-20T12:00:00Z"
  }
}
```
...
## 5. PaaS, Video & Specialized Services API

### 5.1 Cloud Container (PaaS)
* **Base Path:** `/paas/1.25`
* **Endpoints:**
  * `GET /paas/1.25/projects`: List user container projects.
  * `POST /paas/1.25/projects`: Create new namespace/project.
  * `GET/POST /paas/1.25/projects/{project}/apps`: Manage containerized deployments.

### 5.2 Video Platform (VOD) & Live Streaming
* **Base Paths:** `/vod/2.0`, `/live/2.0`
* **Endpoints:**
  * `GET/POST /vod/2.0/channels`: Manage video distribution channels.
  * `POST /vod/2.0/channels/{channel_id}/videos`: Upload and initiate transcoding.
  * `POST /live/2.0/streams`: Create RTMP live streaming endpoints with HLS output.

---

## 6. WordPress Plugin Integration Architecture

The `arv-seller` plugin encapsulates this API specification inside [class-arvan-api-client.php](file:///c:/Users/reza2/Local%20Sites/seller/app/public/wp-content/plugins/arv-seller/includes/class-arvan-api-client.php):
